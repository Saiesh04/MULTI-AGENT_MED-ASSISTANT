...
def validate_medical_output(
    ...
    try:
        # Set session cookie if response object is available
        if response:
            response.set_cookie(key="session_id", value=session_id)
        ...
        if validation_result.lower() == 'yes':
            return {
                ...
                "response": "Thank you for validating the medical output. The diagnosis has been confirmed."
            }
        else:
            validation_message = "**Output requires further review.**"
            if comments:
                validation_message += f"\n\n**Comments from validator:** {comments}"
            
            return {
                "status": "rejected",
                "comments": comments,
                "message": validation_message,
                "response": "The medical output has been flagged for review. Please consult with a healthcare professional."
            }
    except Exception as e:
...
async def transcribe_audio(audio: UploadFile = File(...)):
    ...
    if not config.speech.eleven_labs_api_key or config.speech.eleven_labs_api_key.strip() == "":
        return JSONResponse(
            status_code=503,
            content={"error": "Speech transcription is disabled. Eleven Labs API key not configured."}
        )
...
async def generate_speech(request: SpeechRequest):
    ...
    try:
        print(f"Speech generation request received")
        print(f"Request text: {request.text[:50]}..." if len(request.text) > 50 else f"Request text: {request.text}")
        print(f"Request voice_id: {request.voice_id}")
        ...
        # Check if API key is available
        if not api_key or api_key.strip() == "" or api_key == "your_elevenlabs_api_key_here":
            print("Error: API key not properly configured")
            return JSONResponse(
                status_code=503,
                content={"error": "Speech generation is disabled. Eleven Labs API key not configured."}
            )
        
        # Use default voice if none provided
        if selected_voice_id == "EXAMPLE_VOICE_ID":
            selected_voice_id = config.speech.eleven_labs_voice_id
            print(f"Using default voice: {selected_voice_id}")
        
        print(f"Final voice ID: {selected_voice_id}")
        
        # Define API request to ElevenLabs
        elevenlabs_url = f"https://api.elevenlabs.io/v1/text-to-speech/{selected_voice_id}/stream"
        print(f"API URL: {elevenlabs_url}")
        ...
        free_tier_models = ["eleven_turbo_v2_5", "eleven_turbo_v2", "eleven_flash_v2_5"]
        ...
        for model_id in free_tier_models:
            ...
            print(f"Response status for {model_id}: {response.status_code}")
            
            if response.status_code == 200:
                print(f"Success with model: {model_id}")
                break
            elif response.status_code == 401 and "model_deprecated_free_tier" in response.text:
                print(f"Model {model_id} not available on free tier, trying next...")
                continue
            else:
        ...
        if response.status_code != 200:
            ...
            if response.status_code == 401:
                error_msg = "Invalid API key. Please check your ElevenLabs API key in the .env file."
            elif response.status_code == 429:
                error_msg = "Rate limit exceeded. Please wait or upgrade your ElevenLabs plan."
            elif response.status_code == 422:
                error_msg = "Invalid request format or voice ID not found."
            else:
                error_msg = f"ElevenLabs API error: {response.status_code}"
            
            return JSONResponse(
                status_code=500,
                content={"error": error_msg, "details": response.text, "status_code": response.status_code}
            )
        ...
        return JSONResponse(
            status_code=500,
            content={"error": f"Speech generation failed: {str(e)}", "type": type(e).__name__}
        )

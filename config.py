...
Modified to use Google Gemini API instead of Azure OpenAI for cost-effectiveness.
...
class AgentDecisoinConfig:
    def __init__(self):
        self.llm = ChatGoogleGenerativeAI(
            model="gemini-2.5-flash",  # Using Gemini 2.5 Flash - latest optimized model
            google_api_key=os.getenv("GOOGLE_API_KEY"),
            temperature=0.1,  # Deterministic
            convert_system_message_to_human=True  # Compatibility fix
        )
...
class RAGConfig:
    def __init__(self):
        ...
        self.llm = ChatGoogleGenerativeAI(
            model="gemini-2.5-flash",  # Using Gemini 1.5 Flash for cost-effectiveness
            google_api_key=os.getenv("GOOGLE_API_KEY"),
            temperature=0.3  # Slightly creative but factual
        )

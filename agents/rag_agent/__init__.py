    ...
    def ingest_file(self, document_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if file_extension == '.csv':
                # Process CSV file
                self.logger.info("Detected CSV file - processing with CSV processor...")
                document_chunks = self.csv_processor.process_csv(document_path)
                self.logger.info(f"   CSV processed into {len(document_chunks)} chunks")
            ...
            # Step 5: Create vector store and document store (same for both CSV and PDF)
